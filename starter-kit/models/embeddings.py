...
class BGEpeftEmbedding(Embeddings):
    def __init__(self, model_path, lora_path=None, batch_size=64, **kwargs):
        ...
        self.DEFAULT_QUERY_BGE_INSTRUCTION_EN = "generate a representation for this sentence to retrieve related articles."
    ...
    def embed_query(self, text: str) -> List[float]:
        ...
        """
        text = text.replace("\n", " ")
        encoded_input = self.tokenizer([self.DEFAULT_QUERY_BGE_INSTRUCTION_EN + text], padding=True, truncation=True, return_tensors='pt')
        encoded_input.to(self.device)
        with torch.no_grad():
            model_output = self.model(**encoded_input)
            # Perform pooling. In this case, cls pooling.
